...
import CurriculumInformations from '@/components/front-end/curriculum/curriculum-informations';
import CurriculumPhotoGallery from '@/components/front-end/curriculum/curriculum-photo-gallery';
import CurriculumTagline from '@/components/front-end/curriculum/curriculum-tagline';
...
function IndexPage() {
    ...
    const { data } = props;
    const [curriculumData, setCurriculumData] = useState<Curriculum>({} as Curriculum);
    const [curriculumDataLoading, setCurriculumDataLoading] = useState<boolean>(true);

    useEffect(() => {
        fetch('/api/education/curriculum-data/' + data)
            .then((res) => res.json())
            .then((res) => {
    ...
    return (
        <FrontEndLayout>
            ...
            >
                ...
                >
                    <div className="relative z-10 flex flex-col items-center justify-center px-4 py-20 text-center md:py-40 lg:py-52">
                        ...
                        <h2 className="font-merriweather mb-4 text-3xl font-bold md:text-5xl">{curriculumData.name}</h2>
            ...
            {curriculumDataLoading ? (
                ...
                <>
                    <CurriculumTagline name={curriculumData.name} sub_title={curriculumData.sub_title} introduction={curriculumData.introduction} />
                    <CurriculumPhotoGallery slug={curriculumData.slug} />
                    <CurriculumInformations curriculumData={curriculumData} />
                    <ContactBanner />
                </>
            )}
        </FrontEndLayout>
