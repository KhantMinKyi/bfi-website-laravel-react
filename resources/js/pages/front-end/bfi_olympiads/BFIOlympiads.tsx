import DataList from '@/components/front-end/bfi_olympiads/data-list';
import ContactBanner from '@/components/front-end/home/contact-banner';
import FrontEndLayout from '@/layouts/front-end-layout';
import { BfiOlympiad } from '@/types';
import { motion } from 'framer-motion';
import { useEffect, useState } from 'react';
function BFIOlympiads() {
    const [data, setData] = useState<BfiOlympiad[]>([]);
    const [loading, setLoading] = useState(true);
    useEffect(() => {
        fetch('/api/bfi_olympiads/get-bfi_olympiads-data')
            .then((res) => res.json())
            .then((res) => {
                setData(res.data);
                setLoading(false);
            ...
            >
                ...
                >
                    <div className="relative z-10 flex flex-col items-center justify-center px-4 py-20 text-center md:py-40 lg:py-52">
                        ...
                        <h2 className="font-merriweather mb-4 text-3xl font-bold md:text-5xl">BFI Olympiads</h2>
                        <p className="mb-6 max-w-2xl text-lg md:text-xl">Our path through innovation, challenge, and achievement.</p>
                    </div>
                </motion.div>
            </div>

            <DataList data={data} isLoading={loading} />
            <ContactBanner />
        </FrontEndLayout>
    );
}

export default BFIOlympiads;
